...
- Implement the specification pattern for reusable queries

## Change Tracking & Saving

- Use appropriate change tracking strategies
- Batch `SaveChangesAsync()` calls where possible
- Implement concurrency control for multi-user scenarios
- Use transactions for multiple operations
- Use scoped `DbContext` lifetimes in web apps

## Security

- Avoid SQL injection by using parameterized queries (EF Core handles this by default)
- Be careful with raw SQL queries (`FromSqlRaw`, `ExecuteSqlRaw`)
- Implement appropriate data access permissions at the application layer

## Testing

- Use the in-memory database provider for unit tests
- Create separate testing contexts with SQLite for integration tests
- Mock `DbContext` and `DbSet<T>` for pure unit tests
